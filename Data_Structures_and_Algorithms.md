...
所谓<font color=blue>**程序性能(performance of program)**</font>是指运行这个程序所需要的内存和时间的多少. 用两种方法来确定一个程序的性能, 一个是分析方法, 一个是实验方法. 在性能分析时采用分析方法, 性能测量时使用实验方法.

所谓<font color=blue>**空间复杂度(space complexity)**</font>是指改程序的运行所需内存的大小. 对空间复杂度感兴趣的原因如下:

>   -   如果一个程序要运行在一个多用户计算机系统中, 那么需要指明该程序所需要的内存大小
>   -   任何一个计算机系统上运行的程序, 都需要知道是否有足够的内存可以用来运行该程序.
>   -   一个问题可能有若干个解决方案, 它们对内存的需求各不相同. 
>   -   利用空间复杂度可以估算一个程序所能解决的问题最大可以是什么规模.

所谓<font color=blue>**时间复杂度(time complexity)**</font>是指运行程序所需要的时间. 对时间复杂度感兴趣的原因如下:

>   -   有些计算机需要用户提供程序运行时间的上限, 一旦达到这个上限, 程序将被强制结束. 如果因为数据问题陷入死循环, 可要为时机付出巨额资金. 因此希望时间上限稍大于所期望的运行时间.
>   -   正在开发的程序可能需要一个令人满意的实时响应。利用程序或者程序模块的时间复杂度, 可以判定响应时间是否可以接受, 如果不能接受那就要重新设计算法, 或者为用户提供一个更快的计算机.
>   -   如果一个问题有多种解决方案, 那么具体采用哪一种方案, 主要根据这些方案的性能差异, 对于各种方案的时间和空间性能, 可以采用加权测量方式进行评价.
...
3.   **环境栈空间(environment stack space)**
     ...
     环境栈用来保存暂停的函数和方法在恢复运行时所需要的信息. 例如, 如果函数foo调用了函数goo, 那么至少要保存在函数goo结束时函数foo继续执行的指令地址.
...
>   好久没看了, 服了, 弄了Hyper-v之后虚拟机老卡死....., 重新弄一个算了. 看看有没有其他方法